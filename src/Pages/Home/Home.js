...
function Home() {
  ...
  useEffect(async () => {
    ...
    const docRef = doc(
      db,
      "users",
      auth.currentUser ? auth.currentUser.uid : "asd"
    );
  ...
  const filteredPosts = posts.filter((post) => {
    return (
      post.data.name.toLowerCase().includes(searchField.toLowerCase()) ||
      post.data.status.toLowerCase().includes(searchField.toLowerCase())
    );
  });
      ...
      {filteredPosts.map((post, key) => {
        return (
          <DonkeyDetailed
            key={key}
            name={post.data.name}
            specie={post.data.specie}
            breed={post.data.breed}
            type={post.data.type}
            ...
            before={post.data.before}
            image={post.data.image}
            id={post.id}
            date={post.data.date}
            datex={post.data.datex}
            monthx={post.data.monthx}
            yearx={post.data.yearx}
          />
        );
      })}
    </div>
